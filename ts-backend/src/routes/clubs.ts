...
const asArray = (x: unknown): readonly string[] =>
  Array.isArray(x) ? x.map(String) : x == null ? [] : [String(x)];
...
const findByCode = (clubs: readonly Club[], code: string): Club | undefined =>
  clubs.find((c) => c.code === code);
